model MassPrismaticSystemWithDamping
  ...
  Modelica.Mechanics.Translational.Components.Damper damper1(d = 1) annotation(Placement(visible = true, transformation(origin = {8, 64}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
equation
  connect(damper1.flange_b, prismatic1.axis) annotation(Line(points = {{18, 64}, {18, 22}, {10, 22}}, color = {0, 127, 0}));
  connect(damper1.flange_a, force1.support) annotation(Line(points = {{-2, 64}, {-2, 34}}, color = {0, 127, 0}));
...
end MassPrismaticSystemWithDamping;